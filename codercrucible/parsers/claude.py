"""Claude Code parser for CoderCrucible.

This parser handles Claude Code session JSONL files and converts them
...
from .schema import ParsedSession, SessionStats

logger = logging.getLogger(__name__)


class AnonymizerWrapper:
    """Wrapper around Scout AnonymizerTool that provides anonymization."""
    ...
    def text(self, content: str) -> str:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Anonymizer text() failed: {e}, returning original"
            )
            return content
    ...
    def path(self, file_path: str) -> str:
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Anonymizer path() failed: {e}, returning original"
            )
            return file_path


class PassthroughAnonymizer:
    """A no-op anonymizer for when raw data is needed."""
...
def _summarize_tool_input(
    tool_name: str | None, input_data: Any, anonymizer: AnonymizerWrapper
) -> str:
...
def _extract_user_content(
    entry: dict[str, Any], anonymizer: AnonymizerWrapper
) -> str | None:
    """Extract user message content from an entry."""
...
def _extract_assistant_content(
    ...
    """Extract assistant message content from an entry."""
    msg_data = entry.get("message", {})
        ...
        if block_type == "text":
            ...
            tool_uses.append({
                ...
                "input": _summarize_tool_input(
                    block.get("name"), block.get("input", {}), anonymizer
                ),
            })
...
def _process_entry(
    ...
    """Process a single JSONL entry and add to messages list."""
        ...
        with open(filepath) as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                _process_entry(
                    entry, messages, metadata, stats, anonymizer, include_thinking
                )
    except OSError:
        return None
...
class ClaudeParser(BaseParser):
    ...
    This parser reads Claude Code's JSONL session files and converts them
    to the unified schema format used by CoderCrucible.
    """
    ...
    def __init__(self, claude_dir: Path | None = None):
        ...
        """
        self._claude_dir = claude_dir or _get_claude_dir()
    ...
    def discover(self, base_path: Path | None = None) -> list[dict[str, Any]]:
        """Discover all Claude Code projects with session counts.

        Args:
            base_path: Optional path to Claude Code directory.
                      Defaults to the configured claude_dir.

        Returns:
            A list of project metadata dicts.
        """
        base_dir = base_path or self._claude_dir
        ...
        for project_dir in sorted(projects_dir.iterdir()):
            ...
            projects.append({
                "id": project_dir.name,
                "name": _build_project_name(project_dir.name),
    ...
    def parse(
        self,
        session_id: str,
        anonymizer: AnonymizerWrapper | None = None,
        include_thinking: bool = True,
        anonymize: bool = True,
    ) -> dict[str, Any]:
        """Parse a single Claude Code session.

        Args:
            session_id: The session ID (filename without .jsonl extension).
            anonymizer: Optional AnonymizerWrapper instance. If None and
                       anonymize=True, a default one will be created.
        ...
        Returns:
            A session dict conforming to the unified schema.
        """
        effective_anonymizer: AnonymizerWrapper
        if anonymize:
            effective_anonymizer = anonymizer or AnonymizerWrapper()
        else:
            effective_anonymizer = PassthroughAnonymizer()
        ...
        parsed = _parse_session_file(
            session_file, effective_anonymizer, include_thinking
        )
        if not parsed:
            raise ValueError(f"Failed to parse session: {session_id}")

        parsed["project"] = project_name

        # Ensure stats is a dict for backward compatibility
        if hasattr(parsed.get("stats"), "model_dump"):
            parsed["stats"] = parsed["stats"].model_dump() if hasattr(parsed["stats"], "model_dump") else dict(parsed["stats"])
    ...
    def parse_project(
        ...
        """Parse all sessions for a specific project.

        Args:
            project_id: The project directory name.
            anonymizer: Optional AnonymizerWrapper instance.
            include_thinking: Whether to include thinking blocks.
            anonymize: Whether to anonymize the session content.

        Returns:
            A list of parsed session dicts.
        """
        base_dir = self._claude_dir
        ...
        sessions = []
        project_name = _build_project_name(project_id)
        ...
        for session_file in sorted(project_path.glob("*.jsonl")):
            ...
            if parsed and parsed.get("messages"):
                ...
                if hasattr(parsed.get("stats"), "model_dump"):
                    parsed["stats"] = dict(parsed["stats"])
                sessions.append(parsed)

        return sessions

    def _find_session_file(self, session_id: str) -> Path | None:
        """Find a session file by ID across all projects."""
